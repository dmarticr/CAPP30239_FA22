...
d3.csv("../degrees.csv").then((data) => {
  ...
  let arr1 = d3.map(data, d => d.source), // creates array of source
      arr2 = d3.map(data, d => d.target);
  let names = arr1.concat(arr2); // concat both arrays
  //console.log(names)
  ...
  const chords = d3.chordDirected() // function that we need to create the arcs
