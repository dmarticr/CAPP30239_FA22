...
d3.json('climate.json').then((data) => {      
    ...
    function updateChart(m) {
        ...
        svg.selectAll("foreignObject").remove(); // select all foreign obj and removing them (useful when changing the month)
    ...
    d3.selectAll("select")
        .on("change", function (event) {
            const m = event.target.value; // Event is a change in the page
            updateChart(m); 
        });
});