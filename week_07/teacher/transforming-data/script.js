...
d3.json("a3cleanedonly2015.json").then(data => {
  ...
  let monthData = []; // empty space for months
  ...
  for (let d of data) {
    ...
    let date = d3.timeParse("%x")(d.Date); //parse is in, format is out
    let i = date.getMonth(); //gets the value of the month, 0 is jan

    if(!(i in monthData)) {// if month does not exist, create it with count == 0
      monthData[i] = {
        // month: date.toLocaleString('default', { month: 'long' }),
        month: d3.timeParse("%Y-%m")(date.toISOString().substr(0, 7)), //date.toIO... maps month # to month name
        count: 0
      };
    }

    monthData[i].count++;//else, add the value
  };
