...
Promise.all([
  ...
  // Quantize evenly breakups domain into range buckets / creates scales of colors
  ...
  const path = d3.geoPath();
   ...
   svg.append("g")
     ...
     .join("path")
     .attr("fill", d => (d.id in dataById) ? color(dataById[d.id].rate) : '#ccc') // ccc is grey for places without data
     .attr("d", path)
